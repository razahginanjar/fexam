...
public class InvCountExtraController extends BaseController {
    ...
    @GetMapping
    public ResponseEntity<Page<InvCountExtra>> list(InvCountExtra invCountExtra, @PathVariable Long organizationId, @ApiIgnore @SortDefault(value = InvCountExtra.FIELD_EXTRA_INFO_ID,
    ...
    public ResponseEntity<InvCountExtra> detail(@PathVariable Long extraInfoId, @PathVariable String organizationId) {
    ...
    @Permission(level = ResourceLevel.ORGANIZATION)
    @DeleteMapping
    public ResponseEntity<?> remove(@RequestBody List<InvCountExtra> invCountExtras, @PathVariable String organizationId) {
